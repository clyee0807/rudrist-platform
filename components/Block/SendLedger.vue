...
const portfolioAvailableCurrencies = ref([
    {
        name: "USDT",
        icon: "/_nuxt/assets/img/base_currency/USDT.png",
    },
...
<template>
    <Block :row="row" :col="col" class="flex flex-col gap-y-5">
        <div class="flex items-center">
            <h3 class="flex-1 text-2xl font-medium text-black-100 dark:text-white-100">Send Ledger</h3>
        </div>
        <div class="block-content flex flex-col justify-between w-full flex-1" :style="'--active-color:var('+optionsSteps[0].color+')'">
            <ul class="option-bs flex rounded-lg overflow-hidden bg-white-200 dark:bg-black-100">
                <li v-for="(item, idx) in optionsSteps" :class="{active:idx===0}" class="
                    relative flex items-center justify-center flex-1 h-[2rem] font-medium tracking-widest
                    text-black-100 dark:text-white-100 [&.active]:text-white-100 [&.active]:dark:text-black-100
                    after:absolute after:left-full after:border-[1rem] after:border-solid after:border-transparent
                    before:absolute before:right-full before:border-[1rem] before:border-solid before:border-transparent
                " @click="onChangeStep($event, item.color)">{{ item.name }}</li>
            </ul>
            <DropDown class="text-center text-base font-medium text-black-100 dark:text-white-100" width="100%" height="2.5rem" :items="availablePortfolios"/>
            <div class="flex flex-col justify-between w-full gap-2">
                <DropDown class="text-center text-base font-medium text-black-100 dark:text-white-100" width="100%" height="2rem" :items="portfolioAvailableCurrencies"/>
            ...
            <div class="w-full h-10 overflow-hidden rounded-lg">
